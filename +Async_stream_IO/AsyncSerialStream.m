classdef AsyncSerialStream<Async_stream_IO.IAsyncStream
	...
	methods
		...
		function LocalPort=AsyncInvoke(obj,RemotePort,varargin)
			...
			% Callback function_handle，当远程返回值时调用。如果远程未返回值，Callback将永不会被调用。如果远程返回异常，将调用Callback并提供一个Exception对象作为参数。如
			%  果远程成功返回，将调用Callback并提供一个(:,1)uint8的返回值作为参数。
		...
		function Port=BindFunctionToPort(obj,Function,Port)
			%将本地服务函数绑定到端口，等待远程调用。
			%将任意可调用对象绑定端口上作为服务，当收到消息时调用。远程要调用此对象，需要将所有参数序列化拼接成一个连续的内存块，并且头部加上一个uint8的远程端口号用来接收返回
			% 值，然后将此消息发送到此函数绑定的本地端口。如果远程不期待返回值，应将端口号设为255。
			...
			% ```
			%# 输入参数
			% Function function_handle((:,1)uint8)，当远程调用时，将调用Function(Argument)。如果有返回值，只能有一个返回值，且必须能够typecast为(:,1)uint8。如果
		...
		function PB=Listen(obj,varargin)
			...
			%
			% MessageSize=obj.Listen(Port);
			% %同步监听一个本地端口。调用后将无限等待指定端口，直到其收到消息才会返回此消息的字节数。那之后，用户必须在接下来的代码中立即从基础流读出那个数目的字节。
			% %此监听是同步的，优先于所有异步监听和调用。可以使用此方法监听已被异步监听或调用的端口而不覆盖之前的监听和调用，也可以在此方法等待期间发生的中断中对同一个端口添
			% % 加异步监听和调用。在这些情境下，那个异步监听或调用不会比此同步监听更早收到消息。用户在此方法返回并读完返回的字节数之前不应再次调用此方法、SyncInvoke或结束本
			% % 次代码执行会话，否则行为未定义。
		...
		function Send(obj,Message,ToPort)
			%向远程ToPort发送指定Message。Message必须支持typecast为uint8类型。Message在串口传输时会加上报文头，所以不能多次Send而指望这些消息会自动串联在一起，单个报文必
			% 须串联后一次性交给Send发送。
