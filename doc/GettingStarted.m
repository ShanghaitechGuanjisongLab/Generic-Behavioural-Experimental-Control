...
if questdlg('在 Arduino IDE 中尝试安装了Timers_one_for_all（v3.0.0+）和Quick_digital_IO_interrupt（v3.0.0+）吗？','请确认','成功了','失败了','成功了')=="失败了"
...
NameVersion = sprintf('%s-v%s', Name, Version);
...
MATLAB.IO.MoveFile(fullfile(libsDir,NameVersion), fullfile(libsDir,Name));
end
