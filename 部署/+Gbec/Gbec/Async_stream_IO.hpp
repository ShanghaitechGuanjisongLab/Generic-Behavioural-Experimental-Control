...
struct AsyncStream {
	Stream& BaseStream;
	AsyncStream(Stream& BaseStream) : BaseStream(BaseStream) {}
	...
	void Send(const void* Message, uint8_t Length, uint8_t ToPort);
	...
	inline void Send(const T& Message, uint8_t ToPort) {
		Send(&Message, sizeof(T), ToPort);
	}
	//对象构造后将禁用中断。出于性能考虑，此对象生存期内，用户必须直接使用BaseStream.write()方法发送数据，而不能使用Send；此方法发送的所有数据会被字节串联并视为单条消息。在对象析构前，用户必须恰好已发送Length字节的数据，不能多也不能少，否则行为未定义。对象析构时中断会恢复到构造前的状态。
	...
	inline Exception BindFunctionToPort(T&& Function, uint8_t Port) {
		return Listen(_FunctionListener<std::_FunctionSignature_t<T>>(std::move(Function), BaseStream), Port);
	}
	...
	inline uint8_t BindFunctionToPort(T&& Function) {
		return Listen(_FunctionListener<std::_FunctionSignature_t<T>>(std::move(Function), BaseStream));
	}
...
protected:
	...
	struct _FunctionListener<ReturnType(ArgumentType...) const> {
		_FunctionListener(std::move_only_function<ReturnType(ArgumentType...) const>&& Function, Stream& BaseStream)
...
#pragma pack(pop)
				Send(ReturnMessage{ Exception::Success, _InvokeWithMemoryOffsets<typename _CumSum<std::index_sequence<sizeof(ArgumentType)...>>::type>::Invoke(Function, Arguments.get() + 1) }, Arguments[0]);
			}
			else
