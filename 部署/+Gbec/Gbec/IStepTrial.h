...
struct IStep {
  //会话开始前将被所在回合调用一次，通常用于设置pinMode等初始化状态。如果不需要初始化可以不继承此方法。
  ...
  */
  virtual bool Start(std::move_only_function<void()const>&& FinishCallback) const {};
  // 此方法并非必需实现。对于即时步骤，因为步骤立即完成不会被暂停，不需要实现此方法。对于延时步骤，一般应当实现此方法，否则暂停指令将被忽略。
