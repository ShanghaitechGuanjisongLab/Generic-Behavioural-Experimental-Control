...
inline void Send(const T& Message, uint8_t ToPort, Stream& ToStream = Serial) {
	Send(&Message, sizeof(T), ToPort, ToStream);
}
//对象构造后将禁用中断。出于性能考虑，此对象生存期内，用户必须直接使用ToStream.write()方法发送数据，而不能使用Send；此方法发送的所有数据会被字节串联并视为单条消息。在对象析构前，用户必须恰好已发送Length字节的数据，不能多也不能少，否则行为未定义。对象析构时中断会恢复到构造前的状态。
struct SendSession : protected _InterruptGuard {
	SendSession(uint8_t Length, uint8_t ToPort, Stream& ToStream = Serial);
};
...
inline Exception BindFunctionToPort(T&& Function, uint8_t Port, Stream& ToStream = Serial) {
	return Listen(_FunctionListener<std::_FunctionSignature_t<T>>(std::move(Function), ToStream), Port, ToStream);
}
...
inline uint8_t BindFunctionToPort(T&& Function, Stream& ToStream = Serial) {
	return Listen(_FunctionListener<std::_FunctionSignature_t<T>>(std::move(Function), ToStream), ToStream);
}
...
void RemoteInvoke(uint8_t RemotePort, std::move_only_function<void(Exception Result, ReturnType ReturnValue) const>&& Callback, Stream& ToStream, ArgumentType... Arguments) {
	SendSession const Session{ sizeof(uint8_t) + _TypesSize<ArgumentType...>::value, RemotePort, ToStream };
	ToStream.write(static_cast<char>(Receive([Callback = std::move(Callback)](const std::move_only_function<void(void* Message, uint8_t Size) const>& MessageReader, uint8_t MessageSize) {
		...
		if (MessageSize < sizeof(Exception))
			...
			Callback(*reinterpret_cast<const Exception*>(Message.get()), *reinterpret_cast<const ReturnType*>(reinterpret_cast<const Exception*>(Message.get()) + 1));
	                                         ...
	                                         RemotePort, ToStream)));
	size_t const Written[] = { ToStream.write(reinterpret_cast<char*>(&Arguments), sizeof(Arguments))... };
}
/* 向指定流ToStream远程调用指定RemotePort上的函数，传入参数Arguments。当远程函数返回时，调用Callback，提供调用结果Result和返回值ReturnValue。
		如果远程端口未被监听，Callback将不会被调用。
		如果Result为Corrupted_object_received，说明收到了意外的返回值。这可能是因为远程函数返回值的类型与预期不符，或者有其它远程对象向本地端口发送了垃圾信息。此次远程调用将被废弃。
...
inline typename _NotMof<CallbackType>::type RemoteInvoke(uint8_t RemotePort, const CallbackType& Callback, Stream& ToStream, ArgumentType... Arguments) {
