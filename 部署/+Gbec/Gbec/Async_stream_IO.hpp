...
public:
	...
	// 异步调用指定RemotePort上的函数，传入任意参数Arguments，所有参数类型必须支持直接序列化。此重载不期待远程返回值，所以也不分配或占用本地端口。
	...
	Exception SyncInvokeWithReturn(uint8_t RemotePort, TReturn &ReturnValue, TArgument... Arguments) {
		...
		BaseStream.write(LocalPort);
		size_t const Written[] = { BaseStream.write(reinterpret_cast<char *>(&Arguments), sizeof(Arguments))... };
		uint8_t MessageSize = Listen(LocalPort);
	...
	/*
			同步调用指定RemotePort上的函数，以及任意参数Arguments，所有参数类型必须支持直接序列化。此方法将内部分配一个临时本地端口用于接收远程返回值，然后无限等待，直到收到远程返回消息，然后返回成功或异常。
			此调用是同步的，优先于所有异步监听和调用。可以使用此方法调用已被异步监听或调用的端口，也可以在此方法等待期间发生的中断中对同一个端口添加异步监听或调用。在这些情境下，那个异步监听或调用不会比此同步调用更早收到消息，因为异步监听和调用实际上发生在ExecuteTransactionsInQueue中。用户在此方法返回并读完返回的字节数之前不应再次调用此方法、同步Listen或ExecuteTransactionsInQueue，否则行为未定义。
			同步调用不占用端口。不能用ReleasePort取消同步调用，且同步调用等待中的端口可能被AllocatePort分配。
			谨慎使用此方法。如果中断被禁止，此方法将永不返回。如果远程一直不返回，此方法也将永不返回。
			*/
	...
	Exception SyncInvokeWithoutReturn(uint8_t RemotePort, TArgument... Arguments) {
		...
		uint8_t MessageSize = Listen(LocalPort);
		ReleasePort(LocalPort);
		return MessageSize == sizeof(Exception) ? Read<Exception>() : Exception::Corrupted_object_received;
	}
};
}