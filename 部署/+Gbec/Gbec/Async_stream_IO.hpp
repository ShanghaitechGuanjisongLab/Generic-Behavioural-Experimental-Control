...
namespace Async_stream_IO
{
	...
	{
		Stream &BaseStream;
		SendSession(MessageSize Length, Port ToPort, Stream &BaseStream);
...
#pragma pack(pop)
		...
		MessageSize SyncInvoke(Port RemotePort, TArgument... Arguments)
		{
			...
			{
				SendSession const Session{sizeof(Port) + _TypesSize<TArgument...>::value, RemotePort, BaseStream};
	...
	public:
		...
		void Send(const void *Message, MessageSize Length, Port ToPort) const;
		// 向远程ToPort发送指定Message。这个Message对象应当允许直接序列化。
		template <typename T>
		inline void Send(const T &Message, Port ToPort) const
		{
